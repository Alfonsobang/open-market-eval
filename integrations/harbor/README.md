...
`a-share-point-in-time-qa/` is a Harbor 1.3 filing task that asks an agent to extract one annual-report value, preserve its declared unit, normalize it to yuan, and return exact source provenance.

All four task configurations parse cleanly with Harbor 0.21.0, include an Oracle `solve.sh`, write numeric verifier rewards, package fixtures into the container, and run without network access. Repository CI also runs all deterministic verifiers outside the container.

```bash
harbor run -p integrations/harbor/a-share-point-in-time-qa -a oracle --print-config
```
